...
struct FirebaseAvatarService: RemoteAvatarService {
    ...
    func createAvatar(avatar: AvatarModel, image: UIImage) async throws {
        ...
        let url = try await FirebaseImageUploadService().updateImage(image: image, path: path)
    ...
    func removeAuthorIdFromAvatar(avatarId: String) async throws {
        try await collection.document(avatarId).updateData([AvatarModel.CodingKeys.authorId.rawValue: NSNull()])
    }
    ...
    func incrementAvatarClickCount(avatarId: String) async throws {
        try await collection.document(avatarId).updateData([AvatarModel.CodingKeys.clickCount.rawValue: FieldValue.increment(Int64(1))])
    }
